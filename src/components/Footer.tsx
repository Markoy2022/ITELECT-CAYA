...
const Footer = () => {
  ...
  return (
    <footer className="w-full mt-auto bg-[#E5E5E5] dark:bg-zinc-950 px-8 py-16 flex flex-col items-center transition-colors duration-300">
        ...
        <div className="flex flex-col space-y-6 items-center md:items-end">
          {contacts.map((contact, index) => (
            <div key={index} className="flex items-center gap-4 group">
              <a 
                href={contact.href}
                className="text-[10px] font-bold tracking-[0.3em] text-zinc-500 hover:text-[#BFC873] transition-all"
              >
                {contact.label}
              </a>
              <div className="w-24 h-[1px] bg-black/10 dark:bg-white/10 group-hover:bg-[#BFC873] transition-all"></div>
            </div>
          ))}
        </div>
      </div>
      <div className="w-1/2 h-[1px] bg-black/5 dark:bg-white/5"></div>
      <div className="mt-8 text-[8px] font-bold tracking-[0.4em] text-zinc-400 uppercase">
